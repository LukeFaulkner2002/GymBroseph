...
app.post('/api/autoFillRoutines', async (req, res, next) => {
  ...
  if (muscleGroup == "Upper Body" || randomInput1 == 0) {
    const results1 = await db.collection('exercises').find({ MuscleGroup: "Chest", Accessibility: { $regex: equipment + '.*', $options: 'r' } }).toArray();
    const results2 = await db.collection('exercises').find({ MuscleGroup: "Triceps", Accessibility: { $regex: equipment + '.*', $options: 'r' } }).toArray();
    const results3 = await db.collection('exercises').find({ MuscleGroup: "Shoulders", Accessibility: { $regex: equipment + '.*', $options: 'r' } }).toArray();
    const results4 = await db.collection('exercises').find({ MuscleGroup: "Back", Accessibility: { $regex: equipment + '.*', $options: 'r' } }).toArray();
    const results5 = await db.collection('exercises').find({ MuscleGroup: "Biceps", Accessibility: { $regex: equipment + '.*', $options: 'r' } }).toArray();
    const results6 = await db.collection('exercises').find({ MuscleGroup: "Traps", Accessibility: { $regex: equipment + '.*', $options: 'r' } }).toArray();
    const results7 = await db.collection('exercises').find({ MuscleGroup: "Lats", Accessibility: { $regex: equipment + '.*', $options: 'r' } }).toArray();
    ...
    if (equipment == "Community Gym") {
      ...
      results1 = await db.collection('exercises').find({ MuscleGroup: "Quads", Accessibility: { $regex: equipment + '.*', $options: 'r' } }).toArray();
      results2 = await db.collection('exercises').find({ MuscleGroup: "Hamstrings", Accessibility: { $regex: equipment + '.*', $options: 'r' } }).toArray();
      results3 = await db.collection('exercises').find({ MuscleGroup: "Glutes", Accessibility: { $regex: equipment + '.*', $options: 'r' } }).toArray();
      results4 = await db.collection('exercises').find({ MuscleGroup: "Calves", Accessibility: { $regex: equipment + '.*', $options: 'r' } }).toArray();
    }
  ...
  const results = await db.collection('users').updateOne({ _id: ObjectId(_id) }, { $set: { Routine1: routine1, Routine2: routine2 } });
...
app.post('/api/changePassword', async (req, res, next) => {
  ...
  const db = client.db('gymdb');
  var results = await db.collection('users').updateOne({ _id: ObjectId(_id), Password: blueimp(oldPassword) }, { $set: { Password: blueimp(newPassword) } });
...
app.post('/api/resetPassword', async (req, res, next) => {
  ...
  const db = client.db('gymdb');
  var results = await db.collection('users').updateMany({ Email: email, EmailVerified: "1" }, { $set: { Password: blueimp(newPassword) } });
...
app.post('/api/replaceExercise', async (req, res, next) => {
  ...
  if (_id == "" || routineNum == "" || routineIndex == "" || newExerciseId == "") {
    results == "error";
  }
  else {
    const db = client.db('gymdb');

    const newExercise = await db.collection('exercises').find({ _id: ObjectId(newExerciseId) }).toArray();

    var newArray;

    const user = await db.collection('users').find({ _id: ObjectId(_id) }).toArray();

    if (parseInt(routineNum) == 1) {
      newArray = user[0].Routine1;
    }
    else if (parseInt(routineNum) == 2) {
      newArray = user[0].Routine2;
    }

    if (newArray != '' && newArray != '0') {
      newArray[parseInt(routineIndex)] = newExercise[0];
    }

    if (parseInt(routineNum) == 1) {
      results = await db.collection('users').updateOne({ _id: ObjectId(_id) }, { $set: { Routine1: newArray } });
    }
    else if (parseInt(routineNum) == 2) {
      results = await db.collection('users').updateOne({ _id: ObjectId(_id) }, { $set: { Routine2: newArray } });
    }
  }
...
app.post('/api/searchExercises', async (req, res, next) => {
  ...
  results = await db.collection('exercises').find({
    ExerciseName: { $regex: searchText + '.*', $options: 'i' }, Accessibility: { $regex: equipment + '.*', $options: 'r' }, MuscleGroup: { $regex: muscleGroup + '.*', $options: 'r' }, WarmUpReq: { $regex: warmUpReq + '.*', $options: 'r' }
  }).collation({ "locale": "en_US", "strength": 1, "caseLevel": false }).toArray();
  //Omit id: , { projection: { _id: 0 } }

  for (var i = 0; i < 10; ++i) {
    if (results[i + (parseInt(pageNum) * 10)] != undefined) {
      final.push(results[i + (parseInt(pageNum) * 10)]);
    }
  }
...
app.post('/api/register', async (req, res, next) => {
  ...
  const newPassword = (parseInt(Math.random() * 100000000) % 100000000) + 1;
  const newUser = { FirstName: firstName, LastName: lastName, Username: login, Password: blueimp(password), Email: email, EmailVerified: '0', VerificationPassword: newPassword };
  var error = '';

  try {
    const db = client.db('gymdb');
    var result = await db.collection('users').insertOne(newUser);
    tokenSender(email, result.insertedId, newPassword);
  }
  catch (e) {
    error = e.toString();
  }
...
app.post('/api/list', async (req, res, next) => {
  const db = client.db("gymdb");
  //const results1 = await db.collection('users').deleteMany({});
...
app.post('/api/login', async (req, res, next) => {
  ...
  const db = client.db("gymdb");
  const results = await db.collection('users').find({ Username: login, Password: blueimp(password) }).toArray();
  var error = '';
  var id = -1;
  var fn = '';
  var ln = '';
...
app.post("/api/verify", async (req, res) => {
  ...
  const results = await db.collection('users').updateOne({ VerificationPassword: parseInt(passCode) }, { $set: { EmailVerified: "1" } });
  const results2 = await db.collection('users').find({ VerificationPassword: parseInt(passCode) }).toArray();
  ...
  try {
    jwt.verify(token, "ourSecretKey", async function (err, decoded) {
      if (err) {
        ...
        res.send("Email verified successfully! Now you can reset your password if you ever forget it.");
        var json = jwt.decode(token);
        Userid = json.data;
        const results = await db.collection('users').updateOne({ _id: ObjectId(Userid) }, { $set: { EmailVerified: '1' } });
        console.log(results);
      }
    });
  }
...
/*Commenting out email senders because of risk of github ban?? Just remove this comment and its the same as the server.

//Sends email passcode for verification
async function tokenSender(userEmail, userId, tok) {
  ...
  const mailConfigurations = {
    ...
    text: `Hello There! You have recently registered a GymBroseph account with your email. Please use this passcode to verify your account:
